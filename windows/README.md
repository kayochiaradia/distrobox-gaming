...
| `flycast-hires`/gamescope, Xenia `.xbla` stubs, OpenBOR `.pak` wrapper, Wine launchers | Linux wrapper scripts. Windows uses the plain emulator command (OpenBOR uses ES-DE's per-game-folder layout). The two RetroArch wrappers are replaced natively: `retroarch-snes` by an ES-DE per-game `<altemulator>` that `bootstrap.ps1` writes for SMW widescreen hacks in `snes\no_match` (`AltEmulators` in `esde-systems.psd1`), and `retroarch-atari800` by RetroArch folder options `config\Atari800\<ROM folder>.opt` from `configure-emulators.ps1` |
